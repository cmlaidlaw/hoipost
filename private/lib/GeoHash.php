...
class GeoHash {
  ...
  public function getHash() {
    if(!$this->hash) {
      //cml: these two were throwing an exception on (0,0), which ought to be valid, so changed to an isset() check instead of empty()
      if(!isset($this->latitude)/*empty($this->latitude)*/) throw new Exception("Latitude is required");
      if(!isset($this->longitude)/*empty($this->longitude)*/) throw new Exception("Longitude is required");
