...
try {
    ...
    $email = Common::validateEmail( Common::extractPOSTValue( 'email' ) );
    $password = Common::hash( Common::extractPOSTValue( 'password' ) );
    $admin = Common::validateInt( Common::extractPOSTValue( 'admin' ) );
    if ( $admin !== 1 ) {
        $admin = 0;
    }

    $id = $datastore->createAccount(
        $email,
        $password,
        $admin
    );
    ...
    if ( $id !== false ) {
        ...
        if ( $r->MIMEAcceptType() === 'text/html' ) {
            $r->setHTTPCode( 303 );
            $r->redirect(
                DASHBOARD_REDIRECT_PATH . '/?n=a_create_ok'
            );
            $r->send( null );
            exit;
